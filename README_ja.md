...
`speed_contract_v1.json`に固定したseedで12手を選び、両エンジンで適用・逆順undoと状態復元を検証します。
...
評価値、完了深さ、ノード数、経過時間、hashfullを記録します。v2のサイドカーでは、1手ごとの
割当予算、探索開始前の残り時間、秒読み／加算時間、停止理由（`none`、`budget`、
`external_stop`）も保存します。
