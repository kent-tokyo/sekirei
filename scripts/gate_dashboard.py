...
def list_weights():
    return [name for name, _path in _available_weight_paths()]


def _available_weight_paths():
    """Enumerate regular weight files from the fixed local data directory.

    Request values select from this list later; they are never appended to a
    filesystem path.  Resolving the entries also excludes a symlink that
    points outside the configured data directory.
    """
    ...
    for entry in entries:
        ...
        if os.path.isfile(path) and os.path.dirname(path) == data_root:
            weights.append((entry.name, path))
    return sorted(weights)


def _pipeline_run_directories():
    """Enumerate real pipeline-run directories rooted directly under runs/."""
    ...
    for entry in entries:
        path = os.path.realpath(entry.path)
        if os.path.isdir(path) and os.path.dirname(path) == runs_root:
            run_dirs.append((entry.name, path))
    return run_dirs
...
def list_pipeline_runs():
    ...
    """
    entries = []
    for name, d in _pipeline_run_directories():
        try:
            mtime = os.path.getmtime(d)
        except OSError:
            continue
        entries.append({"id": name, "label": name, "started_at": mtime})
...
def _resolve_pipeline_run_dir(run_id):
    """Select a directory already enumerated from the fixed runs root."""
    if not isinstance(run_id, str):
        return None
    for name, run_dir in _pipeline_run_directories():
        if name == run_id:
            return run_dir
    return None
...
def start_run(payload):
    ...
    """
    available_weights = _available_weight_paths()

    def select_weight(requested):
        if not requested or requested == MATERIAL_SENTINEL:
            return None
        for name, path in available_weights:
            if name == requested:
                return path
        raise ValueError(f"unknown weights file: {requested}")

    e1 = payload.get("engine1_weights") or ""
    e2 = payload.get("engine2_weights") or ""
    e1_path = select_weight(e1)
    e2_path = select_weight(e2)
    ...
    run_sequence = _next_run_seq
    _next_run_seq += 1
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    stem = f"{timestamp}_gate_{run_sequence}"
