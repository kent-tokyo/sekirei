...
impl Case {
    ...
    fn calibrate(&mut self) {
        // At least 10ms per sample, with a bounded iteration count. Allocation,
        // lazy table initialization and buffer growth are warmed before timing.
        (self.operation)();
        while self.timed() < Duration::from_millis(10) && self.iterations < 1 << 24 {
            self.iterations *= 2;
        }
    }
}
...
pub(super) fn run() {
    ...
    println!(
        "samples=21;target_sample_ms=10;weights=default_lcg;global_initialization=excluded;debug_assertions={}",
